...
export default class Street extends Cell implements Property {
  ...
  constructor(type: string, name: string, russianName: string, belarusianName: string,
    position: number, cost: number, rent: number, rentWithOneHouse: number,
    rentWhithTwoHouses: number, rentWithTreeHouses: number, rentWhithFourHouses: number,
    rentWhithHotel: number, houseCost: number, kitSize: number, kitId: number, pledgePrice: number,
